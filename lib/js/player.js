...
import QLibrary from "../components/QLibrary.vue";
...
import QQuery from "../components/QQuery.vue";
...
import QSongListing from "../components/QSongListing.vue";
...
class Qmxtr{
	constructor(settings){
		...
		this.LibraryPlaylist = new LibraryPlaylist(this);
		this.playlist = {
			"queue": this.queuePlaylist,
			"library": this.LibraryPlaylist
		};
		this.commands = [];
		this.loadQueue = [];
		this.playing = false;
	...
	set shuffle(v){
		...
		let newQueue = (v) ? shuffle(this.queue) : this.originalQueue.slice();
		if(this.current) {
	...
	addToQueueWrapped(wrapper) {
		...
		if(wrapper.isProvider) {
			wrapper.load().then((v) => this.addToQueueWrapped(v));
			return;
		}

		if(this.originalQueue.find((v) => v.src === wrapper.src)) return false;

		this.hookAdded(wrapper);
	...
	hookAdded(audio) {
		this.playlist.library.add(audio);
	}
	...
	play(){
		...
		if(this.pointer >= this.queue.length) this.pointer = this.queue.length - 1;
	...
	install(Vue){
		...
		Vue.component('q-song-listing', QSongListing);
	...
	get exportData() {
		const exportData = {};
		exportData.shuffle = this.shuffle;
		exportData.repeat = this.repeat;
		exportData.originalQueue = this.originalQueue.map((v) => v.src);
		exportData.queue = this.queue.map((v) => v.src);
		exportData.pointer = this.pointer;
		exportData.playlist = {};
		exportData.audio = {};
		...
		Object.keys(this.playlist).forEach((id) => {
			...
			if(!playlist.isQueue) {
				exportData.playlist[id] = playlist.exportData;
			}
		});

		this.library.forEach((v) => {
			exportData.audio[v.src] = v.exportData;
		});
...
class AudioWrapper {
	...
	get exportData() {
		...
		['title', 'author', 'src', 'albumArt', 'video', 'date'].forEach((v) => {
			obj[v] = this[v];
		});
...
class PlaylistBase {
	...
	reorderIndex(currentIndex, target) {
		...
		const audio = this.content.splice(currentIndex, 1)[0];
		this.content.splice(target, 0, audio);
		this.propagate();
	}

	findIndex(src) {
		const currentIndex = this.content.findIndex((v) => v.src === src);
	...
	get exportData() {
		...
		exportData.content = this.content.map((v) => v.src);

		return exportData;
	}
}

class Playlist extends PlaylistBase {
	constructor(...args) {
		super(...args);
...
class LibraryPlaylist extends PlaylistBase {
	constructor(player, content = []) {
		super(player, 'Library', 'library', content);
	...
	remove(src) {
		const removable = Object.keys(this.player.playlist).every((k) => {
			const playlist = this.player.playlist[k];

			if(playlist.isLibrary) return true;

			return playlist.content.every((v) => v.src !== src);
		});
...
class MusicProvider {
	...
	async query(searchQuery, from, limit) {
		return {
			total: 0,
			content: []
		};
...
class ProviderItem {
	constructor(provider, id, {title, author, albumArt}) {
		this.provider = provider;
		this.id = id;
		this.title = title;
		this.author = (author === undefined) ? "Various Artists" : author;
		this.albumArt = albumArt;
		this.isProvider = true;
	}

	async load() {
		return await this.provider.load(this);
	}
}

Qmxtr.AudioWrapper = AudioWrapper;
Qmxtr.Lyric = Lyric;
Qmxtr.PlaylistBase = PlaylistBase;
Qmxtr.Playlist = Playlist;
Qmxtr.LibraryPlaylist = LibraryPlaylist;
Qmxtr.QueuePlaylist = QueuePlaylist;
Qmxtr.MusicProvider = MusicProvider;
Qmxtr.ProviderItem = ProviderItem;
