...
class Qmxtr{
	constructor(settings){
		...
		this.vuexMutationDefinitions = {
			...
			updatePlaylist(state, {id, playlist}) {
				...
				else state.playlist[i] = playlist;
			}
		};
	...
	newPlaylist(title) {
		const id = sha256(`${title}${Date.now()}${Math.random()}`);
		const playlist = new Playlist(this, title, []);

		this.playlist[id] = playlist;
		this.emit('updatePlaylist', {id, playlist});
	}
...
class Playlist {
	...
	propagate() {
		this.player.emit('updatePlaylist', {playlist: this.content, id: this.id});
	}
}
